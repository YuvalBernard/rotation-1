...
Sim.offset            = 500;                % offset range in ppm
...
Sim.B1            =     500/gamma_;                 % standard B1 value in µT
Sim.Trec          =     5;                  % standard recover time in s
...
Sim.fB=0.02;  % rel. conc 10mM/111M
Sim.kBA=99.56;   % exchange rate in Hz ( the fast one, kBA is calculated by this and fB)
Sim.dwB=0;     % ppm  relative to dwA
Sim.R1B=2;      % R1B relaxation rate [Hz]
Sim.R2B=19000;     % R2B relaxation rate [Hz]

% second CEST pool D (amide)
Sim.fD=0.0000;  % rel. conc 10mM/111M
...
if Sim.all_offsets == 1
  ...
  % Space.tp    = [0.5 1 2 3 4];
   Space.B1    = [500 750 1000 1250 1500]/gamma_;
