...
% this creates a stack of Z-spectra Z_x and teh parameters struct P
 [w_x, Z_x, P, rowname, Ztab]=LOAD_xls_qCEST_2_Ztab({'LP30_323K'}); % 323K
...
rowname= Ztab.Properties.RowNames{ii};
...
Sim.kBA=284;   % exchange rate in Hz ( the fast one, kBA is calculated by this and fB)
Sim.dwB=-260;     % ppm  relative to dwA
Sim.R1B=1;      % R1B relaxation rate [Hz]
Sim.R2B=27000;     % R2B relaxation rate [Hz]

% second CEST pool D (amide)
Sim.n_cest_pool=2;
Sim.fD=0;  % rel. conc 10mM/111M
...
T.lowerB      = [   Sim.dwB-500    Sim.fB*0.1  Sim.kBA/1000     0      ];
T.upperB      = [   Sim.dwB+500    Sim.fB*10    Sim.kBA*1000    500000     ];
