function [Z,A,domain,varargout] = CEST_ode45(T1a,T2a,T1b,T2b,kb,M0a,M0b,dwa,db,w1,varargin)
...
if nargout < 4 % No dynamics analysis (t_ss) requested.
    for j = 1:length(dwa)
        ...
        K = -[-(R2a+ka) dwa(j) 0 kb 0 0;
            ...
            0 0 ka 0 -w1 -(R1b+kb)];
        if isempty(varargin)
            tmax = 7/min(abs(real(eig(K))));
        else
            tmax = tp;
        end
        
        sol = ode45(@(t,y) -K*y + b,[0 tmax],M0);
        y_tmax = deval(sol,tmax);
        Z(j) = y_tmax(3);
        ...
        if dwa(end) ~= -dwa(1) && j >= idx && j <= idx+domain
            ...
            if isempty(varargin)
                tmax = 7/min(abs(real(eig(K_))));
            else
                tmax = tp;
            end
            
            sol = ode45(@(t,y) -K_*y + b,[0 tmax],M0);
            y_tmax = deval(sol,tmax);
            A(j) = (y_tmax(3) - Z(j))/M0a;
        end
    end
    if dwa(end) == -dwa(1)
